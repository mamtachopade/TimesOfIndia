...
public class HomePage extends WebDriverBaseTestPage<WebDriverTestPage> {
	...
	@FindBy(locator = "timesofindia.topstory.list")
	public List<QAFWebElement> topstorylist;
	...
	@FindBy(locator = "toi.upper.header.city.title")
	private QAFWebElement upperHeaderCityTitle;

	@FindBy(locator = "toi.city.pune.text.title")
	private QAFWebElement CityPuneTextTitle;

	@FindBy(locator = "toi.city.list")
	private List<QAFWebElement> toiMetroCityList;
	...
	public List<QAFWebElement> getTopStoryList() {
		return topstorylist;
	}
	...
	public QAFWebElement getUpperHeaderCityTitle() {
		return upperHeaderCityTitle;
	}

	public QAFWebElement getCityPuneTextTitle() {
		return CityPuneTextTitle;
	}

	public List<QAFWebElement> getToiMetroCityList() {
		return toiMetroCityList;
	}
	...
	public int getListOfTopStories() {
		int size = getTopStoryList().size();
		System.out.println("TopStory size count is " + size);
		for (QAFWebElement element : getTopStoryList()) {
	...
	public void displayMetroCitiesList() {
		CommonSteps.mouseHoverToElement(getUpperHeaderCityTitle());
		QAFTestBase.pause(4000);
		for (WebElement city : getToiMetroCityList()) {

			if (city.getText().equalsIgnoreCase("OTHER CITIES")) {
				break;
			}
			if (!(city.getText().equalsIgnoreCase("METRO CITIES")))
				Reporter.log("Metro Cities : " + city.getText());
		}
	}

	public void clickOnCity(String cityName) {
		QAFWebElement city = CommonSteps.getCityByText(cityName);
		CommonSteps.clickUsingJS(city);
	}
}
